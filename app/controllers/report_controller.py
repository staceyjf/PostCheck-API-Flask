...
bp = Blueprint('report', __name__, url_prefix='/api/v1/reporting', description="Operations on reporting")
...
class Report(MethodView):
    ...
    def get(self):
        ...
        Retrieves a reporting dashboard for informtion across property prices across Austrailian suburbs
        #### Responses
        ...
        """
        try:
            all_data = process_property_data()  # 5538 key/value pairs 
            count = len(all_data)
            current_app.logger.info(f"{count}")
